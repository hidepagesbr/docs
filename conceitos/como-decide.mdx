...
<AccordionGroup>
  ...
  <Accordion title="Inteligência de IP: VPN, proxy e datacenter" icon="server">
    IPs de **datacenter** (servidores de nuvem) e de **VPNs/proxies** não costumam ser de compradores comuns. Esse é o sinal por trás da regra **Bloquear Conexões Suspeitas** (veja abaixo), que você ativa na campanha.
  </Accordion>
...
| **Bloquear Conexões Suspeitas** | Manda VPNs, proxies e datacenters para a **Gray** (ou White). **Recomendado ligar**, e travado em ligado para a maioria das fontes (veja [fontes de tráfego](/campanhas/fontes-de-trafego)). |
...
Cada acesso vira uma linha nos **Logs de Acesso** do Dashboard, com a página servida (**Status**) e o **Motivo** da decisão.

<Print src="painel/logs" alt="Logs de Acesso com visitas classificadas como White por Bot e Gray por Fonte de tráfego" />

| Motivo | O que significa |
|---|---|
| **Aprovado** | Passou em tudo. Viu a Black Page. |
| **Parâmetro ausente** | Chegou sem o hash HPID no link. |
| **Bot** | Robô, crawler ou navegador automatizado. |
| **Automação** | Sinais de automação de navegador. |
| **Headers suspeitos** | Cabeçalhos da requisição que um navegador real não envia. |
| **Região de bot** | Rede ou região associada a times de revisão. |
| **Verificação de ad** | Ferramenta de verificação de anúncios. |
| **Suspeito** | Conexão suspeita (VPN, proxy, datacenter) com a regra ligada. |
| **Fonte de tráfego** | Não veio da rede declarada na campanha. |
| **País**, **Idioma**, **Dispositivo** | Fora das regras que você configurou. |

Isso deixa você **ajustar as regras com base em dados reais**. Veja como filtrar e ler os logs em [Dashboard e logs](/painel/dashboard).
